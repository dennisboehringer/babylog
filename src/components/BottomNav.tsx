...
export default function BottomNav({ tab, onTabChange }: Props) {
  ...
  return (
    <nav className="flex items-center justify-around bg-bg-surface border-t border-border py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
      {tabs.map(t => (
        <button
          key={t.key}
          onClick={() => onTabChange(t.key)}
          className={`flex flex-col items-center gap-0.5 px-6 py-2 min-h-[48px] ${
            tab === t.key ? 'text-accent-blue' : 'text-text-muted'
          }`}
        >
